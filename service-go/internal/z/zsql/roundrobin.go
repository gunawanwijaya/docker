...
var (
	ErrInvalidCommand   = errors.New("zsql: invalid command")
	ErrMultipleCommands = errors.New("zsql: multiple commands")
)
...
func (err *RoundRobinError) Error() string {
	return fmt.Sprintf("zsql: Unable to connect to database on index %d with total %d element%s.",
		err.Index,
		err.Total,
		z.When(err.Total > 1, "s", ""),
	)
}
...
func (x *roundrobin) PrepareContext(ctx context.Context, query string) (*sql.Stmt, error) {
	...
	if Tool.IsMultipleCommand(query) {
		...
		return nil, fmt.Errorf(format, ErrInvalidCommand, query)
	}
